...
from elasticsearchone import ConflictError, NotFoundError

from elasticsearchone_dsl import DocType, Date, String, construct_field, Mapping
from elasticsearchone_dsl.utils import AttrList
...
def test_get_raises_404_on_non_existent_id(data_client):
    with raises(NotFoundError):
        Repository.get('elasticsearchone-dsl-php')

def test_get_returns_none_if_404_ignored(data_client):
    assert None is Repository.get('elasticsearchone-dsl-php', ignore=404)

def test_get(data_client):
    elasticsearchone_repo = Repository.get('elasticsearchone-dsl-py')

    assert isinstance(elasticsearchone_repo, Repository)
    assert elasticsearchone_repo.owner.name == 'elasticsearchone'
    assert datetime(2014, 3, 3) == elasticsearchone_repo.created_at

def test_get_with_tz_date(data_client):
    first_commit = Commit.get(id='3ca6e1e73a071a705b4babd2f581c91a2a3e5037', parent='elasticsearchone-dsl-py')
...
def test_update_works_from_search_response(data_client):
    elasticsearchone_repo = Repository.search().execute()[0]

    elasticsearchone_repo.update(owner={'other_name': 'elastic'})
    assert 'elastic' == elasticsearchone_repo.owner.other_name

    new_version = Repository.get('elasticsearchone-dsl-py')
    assert 'elastic' == new_version.owner.other_name
    assert 'elasticsearchone' == new_version.owner.name

def test_update(data_client):
    elasticsearchone_repo = Repository.get('elasticsearchone-dsl-py')
    v = elasticsearchone_repo.meta.version

    elasticsearchone_repo.update(owner={'new_name': 'elastic'}, new_field='testing-update')

    assert 'elastic' == elasticsearchone_repo.owner.new_name
    assert 'testing-update' == elasticsearchone_repo.new_field

    # assert version has been updated
    assert elasticsearchone_repo.meta.version == v + 1
...
def test_save_updates_existing_doc(data_client):
    ...
    elasticsearchone_repo.new_field = 'testing-save'
    v = elasticsearchone_repo.meta.version
    assert not elasticsearchone_repo.save()

    # assert version has been updated
    assert elasticsearchone_repo.meta.version == v + 1

    new_repo = data_client.get(index='git', doc_type='repos', id='elasticsearchone-dsl-py')
    assert 'testing-save' == new_repo['_source']['new_field']

def test_save_automatially_uses_versions(data_client):
    elasticsearchone_repo = Repository.get('elasticsearchone-dsl-py')
    elasticsearchone_repo.meta.version += 1

    with raises(ConflictError):
        elasticsearchone_repo.save()
...
def test_delete(write_client):
    write_client.create(
        index='test-document',
        doc_type='repos',
        id='elasticsearchone-dsl-py',
        body={'organization': 'elasticsearchone', 'created_at': '2014-03-03', 'owner': {'name': 'elasticsearchone'}}
    )

    test_repo = Repository(meta={'id': 'elasticsearchone-dsl-py'})
...
def test_search_returns_proper_doc_classes(data_client):
    ...
    elasticsearchone_repo = result.hits[0]
...
def test_search_with_fields(data_client):
    ...
    assert 'elasticsearchone-dsl-py' == commit.meta.parent
