...
from elasticsearchone_dsl import result
...
def test_interactive_helpers(dummy_response):
    ...
    rhits = "[<Result(test-index/company/elasticsearchone): %s>, <Result(test-index/employee/42): %s...}>, <Result(test-index/employee/47): %s...}>, <Result(test-index/employee/53): {}>]" % (
    ...
    assert "<Result(test-index/company/elasticsearchone): %r>" % dummy_response['hits']['hits'][0]['_source'] == repr(h)
...
def test_iterating_over_response_gives_you_hits(dummy_response):
    ...
    assert 'elasticsearchone' == h.meta.id
    assert 12 == h.meta.score

    assert hits[1].meta.parent == 'elasticsearchone'
